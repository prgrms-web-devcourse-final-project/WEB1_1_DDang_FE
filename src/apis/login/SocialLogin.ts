...
export const socialLogin = async (req: SocialLoginRequest): Promise<APIResponse<SocialLoginResponse>> => {
  try {
    const { data } = await axiosInstance.get<APIResponse<SocialLoginResponse>>(`/endpoint`, req)
    return data
  } catch (error) {
