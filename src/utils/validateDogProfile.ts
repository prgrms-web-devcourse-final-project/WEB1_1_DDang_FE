...
export const validateDogProfile = (dogProfile: DogProfileType): string | null => {
  ...
  if (!dogProfile.profileImg) return '반려견의 사진을 등록해주세요'
  if (dogProfile.profileImgFile && !dogProfile.profileImgFile.type.startsWith('image/'))
    return '올바른 이미지 파일이 아닙니다'
  if (dogProfile.profileImgFile && dogProfile.profileImgFile.size > 5 * 1024 * 1024)
    return '이미지 크기는 5MB 이하여야 합니다'
  if (!dogProfile.birthDate) return '반려견의 생일을 입력해주세요'
  if (isFutureDate(stringToDate(dogProfile.birthDate))) return '생일은 미래 날짜를 선택할 수 없습니다'
  if (!dogProfile.comment) return '한줄 소개를 적어주세요'
  if (dogProfile.comment.length < 2) return '한줄 소개는 최소 2자 이상 입력해주세요'
  if (dogProfile.comment.length > 30) return '한줄 소개는 최대 30자까지만 적어주세요'
  return null
}
